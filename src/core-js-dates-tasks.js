...
function getDayName(date) {
  const dayObj = new Date(date).getUTCDay();
  const days = [
    'Sunday',
    'Monday',
    'Tuesday',
...
function getNextFriday(date) {
  ...
  const dayObj = newDate.getUTCDay();
  let daysToAdd;
  if (dayObj < 5) {
    daysToAdd = 5 - dayObj;
  } else if (dayObj === 5) {
  ...
  newDate.setDate(newDate.getUTCDate() + daysToAdd);
  return newDate;
}
...
function getWeekNumberByDate(date) {
  ...
  const timeDiff = target.getTime() - startYear.getTime();
